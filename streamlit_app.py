...
if uploaded_file is not None:
    # 年月の入力
    yyyymm = st.text_input("年月を入力してください (例: 202501)", "202501")
    ...
    if st.button("変換実行"):
        try:
            ...
            # １行ごとに分割
            df2 = df1.join(df1["text"].str.split("\n", expand=True)).rename(columns={0: "day"})
            
            # 日付を数値に変換
            df2["day"] = pd.to_numeric(df2["day"], errors="coerce")
            
            # 不要列を削除
            df2 = df2.dropna(subset="day").astype({"day": int}).drop(["row", "text"], axis=1)
            ...
            df3[["type", "name"]] = df3["data"].apply(lambda s: pd.Series(([""] + s.split())[-2:]))
            ...
            # kindに変換
            df4["kind"] = (
                df4["type"].map({"救": 1, "整": 4, "内": 5, "小": 7, "島": 9}).fillna(1).astype(int)
            )
            ...
            # 整形外科
            df4["time"] = df4["time"].mask(df4["time"].isna() & (df4["kind"] == 4), "08:30～17:30")
            
            # 内科
            df4["time"] = df4["time"].mask(df4["time"].isna() & (df4["kind"] == 5), "09:00～17:30")
            ...
            # 島しょ部
            df4["time"] = df4["time"].mask(df4["time"].isna() & (df4["kind"] == 9), "09:00～17:00")
